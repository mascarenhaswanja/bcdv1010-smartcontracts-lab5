...
contract("StandardERC20", (accounts) => {
  ...
  before(async () => {
    ...
    assert(
      ...
      .isEqualTo(new BigNumber(tokenSupplyExpected)),
      "Token supply is not as expected"
    );
  });

  it("test balanceOf()", async () => {
    const tokenBalanceInCreator = await standardERC20Instance.balanceOf.call(creator);
    assert(
      new BigNumber(tokenBalanceInCreator)
      .isEqualTo(
  ...
  it("test transfer()", async () => {
    ...
    truffleAssert.eventEmitted(tx, "Transfer", (obj) => {
      return (
        obj.from === creator &&
        obj.to === receipient1 &&
        new BigNumber(receipient1Amount)
        .isEqualTo(new BigNumber(obj.value))
      );
    });
    assert(
      new BigNumber(receipient1Balance)
      .isEqualTo(receipientBalFromContract),
...
it("test increaseAllowance()", async () => {
    ...
    assert(
      newAllowanceSpender.isEqualTo(allowanceIncreaseSpender),
